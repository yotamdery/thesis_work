...
class Model_behave:
    def __init__(
            ...
            beta_school= 0.515,
            activity_vec = visitors_covid,      # The workplace vector itself
            is_activity = lockdown_vec,           # Holds the times of lockdown, to activate the activity vector
        ...
        self.is_activity = is_activity      # 1 or 0, same as the previous the lockdown vec
    ...
    def predict(
        ...
        for t in np.arange(running_time_model):
            ...
            contact_force_matrix, vector_to_plot = self.\
                calculate_force_matrix(final_contact_matrix, t)
            ...
            lambda_t = (self.alpha_variant_vec[t]) * self.delta_variant_vec[t] * self.omicron_variant_vec[t] * \
                    ((beta_activity * self.activity_vec[t])**self.is_activity[t]) * (beta_school ** self.is_school[t]) * \
                ...
                # (beta_lockdown ** self.is_lockdown[t])
    ...
    def calculate_force_matrix(self, final_contact_matrix, t):
        ...
        force_contact_infectious = final_contact_matrix.T.dot((
                self.Is_1[-1][self.ind.risk_dict['High']] * ((1 -  0)) +
                self.Is_1[-1][self.ind.risk_dict['Low']] * ((1 -  0)) +
                self.Ia_1[-1][self.ind.risk_dict['High']] +
                self.Ia_1[-1][self.ind.risk_dict['Low']] +
                self.Is_2[-1][self.ind.risk_dict['High']] * ((1 - (0))) +
                self.Is_2[-1][self.ind.risk_dict['Low']] * ((1 - (0))) +
                self.Ia_2[-1][self.ind.risk_dict['High']] +
                self.Ia_2[-1][self.ind.risk_dict['Low']]
        )).reshape(-1)
        vector_to_plot = (self.Is_1[-1][self.ind.risk_dict['High']] * ((1 - (0))) +
                          self.Is_1[-1][self.ind.risk_dict['Low']] * ((1 - (0))) +
                          self.Ia_1[-1][self.ind.risk_dict['High']] +
                          self.Ia_1[-1][self.ind.risk_dict['Low']] +
                          self.Is_2[-1][self.ind.risk_dict['High']] * ((1 - (0))) +
