...
SIM_DIR = '/Users/yotamdery/Old_Desktop/git/SEIR_model_COVID-main/notebooks/Yotams/simulations/'
LETTERS_FOR_FIGURES = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']

def national_calibration_with_omicron(raw_data: pd.DataFrame, model_pred: pd.Series):
    """Plotting the calibration for total new cases with the omicron variant"""
    # Plotting
    fig = go.Figure()

    # Calibration phase predictions:
    fig.add_trace(go.Scatter(x=raw_data['date'], y=raw_data[raw_data['date'] < '2021-10-26']['absolute_cases'],
                             hoverlabel=dict(namelength=-1),
                             mode='markers', name='Raw data', marker=dict(size=1.5, color='darkblue', line_width=1))
                  )
    fig.add_trace(go.Scatter(x=raw_data['date'], y=model_pred.values[:529], hoverlabel=dict(namelength=-1),
                             line=dict(width=3.5, color='rgba(255, 82, 93, .9)'), name='Model predictions',
                             )
                  )

    # Validation phase predictions:
    fig.add_trace(go.Scatter(x=raw_data[raw_data['date'] > '2021-10-25']['date'], y=model_pred.values[529:],
                             hoverlabel=dict(namelength=-1), line=dict(width=3.5, color='rgba(255, 82, 93, .9)'),
                             )
                  )
    fig.add_trace(go.Scatter(x=raw_data[(raw_data['date'] > '2021-10-25')]['date'], hoverlabel=dict(namelength=-1),
                             y=raw_data[(raw_data['date'] > '2021-10-25') & (raw_data['date'] < '2022-03-18')]['absolute_cases'],
                             mode='markers', marker=dict(size=1.5, color='darkblue', line_width=1),
                             ))

    # Adding Vlines for the omicron time period
    # fig.add_vline(x=datetime.datetime.strptime("2022-01-01", "%Y-%m-%d").timestamp() * 1000,
    #               annotation_text="omicron<br>starts",
    #               annotation_position="top left", annotation_font_size=12, line_dash="dash", line_width=1)
    # fig.add_vline(x=datetime.datetime.strptime("2022-03-18", "%Y-%m-%d").timestamp() * 1000,
    #               annotation_text="omicron<br>finish",
    #               annotation_position="top right", annotation_font_size=12, line_dash="dash", line_width=1)

    fig.add_vline(x=datetime.datetime.strptime("2021-10-25", "%Y-%m-%d").timestamp() * 1000, line_width=1,
                  yref="paper", y1=1.1)
    # Adding Letter annotation for the paper
    fig.add_annotation(xref="paper", yref="paper", x=-0.05, y=1.1,
                       text="<b>A",
                       showarrow=False,
                       font=dict(
                           family="sans-serif",
    ...
    # Updates
    fig.update_layout(height=600, width=1200, xaxis_title="<b> Date", yaxis_title="<b> Daily reported cases [#]",
                      font=dict(size=24), font_family="sans-serif", template='plotly_white', showlegend= True,
        #               title={
        #     'text': '<b>Country level cases, calibration & validation',
        #     'font_size': 42,
        #     'y': 0.9,
        #     'x': 0.5,
        #     'xanchor': 'center',
        #     'yanchor': 'bottom'
        # },
                      legend=dict(title='', traceorder='normal', bgcolor='rgba(0,0,0,0)',
                                  borderwidth=0.0, x=0.001, y=0.999,
                                  font=dict(family='sans-serif', color='black', size=26))
                      )
    fig.update_xaxes(showgrid=False, showticklabels=True, tickfont=dict(family='sans-serif', size=24, color="black"),
                     range= ['2020-05-15', '2022-03-17'], ticklen=1, showline=True, linewidth=1.5, linecolor='black',
                     mirror= True)
    fig.update_yaxes(showgrid=False, showline=True, linewidth=1.5, linecolor='black', showticklabels=True,
                     range= [0, max(np.max(model_pred), np.max(raw_data['absolute_cases'])) + 2000], mirror= True,
                     ticks='outside', ticklen=4, tickwidth=2, tickfont=dict(family='sans-serif', size=24))

    fig.write_image("national_calibration_with_omicron.jpeg")
    fig.show()

def plot_univariate_sensitivity(low_values: pd.Series, high_values: pd.Series, drop_value: Union[int, float],
                                uncertainty_factors: List[str], morbidity_or_mortality: str, file_name: str):
    """This function creates the plot of the univariate sensitivity analysis"""
    # Extracting relevant info from the file_name parameter
    cases_or_hosp = file_name.split('_')[1]
    with_or_without_test = file_name.split('_')[-2] + ' ' + file_name.split('_')[-1]
    fig = make_subplots(x_title='<b>Yearly averted {} / 1K individuals<br>{}-based {}'
                        .format(cases_or_hosp, morbidity_or_mortality, with_or_without_test),
                        y_title='<b>Uncertainty factors')
    # Normalizing reported cases & hospitalizations to be per 1,000 people
    norm_value = pop_israel/1000
    low_values_scaled = low_values/norm_value
    high_values_scaled = high_values/norm_value
    drop_value_scaled = drop_value/norm_value
    #drop_value_scaled = drop_value  # Already scaled!
    # Morbidity data
    fig.add_trace(go.Bar(x=high_values_scaled, y=uncertainty_factors, marker_color='rgb(158,202,225)',
                         name='{}_high'.format(morbidity_or_mortality),  marker_line_color='rgb(8,48,107)',
                         marker_line_width=1.5, opacity=0.7, orientation='h', base=drop_value_scaled)
                  )

    fig.add_trace(go.Bar(x=low_values_scaled, y=uncertainty_factors, marker_color='crimson',
                         name='{}_low'.format(morbidity_or_mortality), marker_line_color='red',
                         marker_line_width=1.5, opacity=0.7, orientation='h', base=drop_value_scaled)
                  )

    fig.add_vline(x=drop_value_scaled, line_width=1, line_color='black', line_dash='dash',
                  annotation=dict(font_size=24, text="base case"), annotation_position='top'
                  )
    fig.update_layout(
        template='plotly_white', showlegend=False, height=800, width=1100,
        barmode='group', bargap=0.4,
        margin=dict(l=320, b=120)
    )
    fig.update_xaxes(ticks='outside', ticklen=4, tickwidth=2, tickfont=dict(family='sans-serif', size=26),
                       showline=True, linewidth=1.5, linecolor='black', showgrid=False)#, ticksuffix='K')
    fig.update_yaxes(tickfont=dict(family='sans-serif', size=26),
                      showgrid=False)  # , showline=True, linewidth=1.5, linecolor='black')
    # Adding Letter annotation for the paper
    if 'with' in file_name.split('_'):
        fig.add_annotation(xref="paper", yref="paper", x= -0.35, y= 1.1,
                            text="<b> {}".format(LETTERS_FOR_FIGURES.pop(0)),
                            showarrow=False,
                            font=dict(
                                family="sans-serif",
                                size=46,
    ...
    # Accessing directly to the x_title and y_title to change them:
    fig['layout']['annotations'][0]['font']['size'] = 28
    fig['layout']['annotations'][1]['font']['size'] = 30
    # Accessing directly to the x location of the y_title and change it:
    fig['layout']['annotations'][0]['yshift'] = -45
    fig['layout']['annotations'][1]['xshift'] = -270

    # Accessing the annotation of the vline (note the index [2] that designated for accessing specifically that annotation)
    # fig_['layout']['annotations'][2]['yshift'] = -10
    fig.write_html("tornado_{}_{}.html".format(morbidity_or_mortality, file_name.replace(" ", "_")))
    fig.write_image("tornado_{}_{}.jpeg".format(morbidity_or_mortality, file_name.replace(" ", "_")))
    fig.show()
def plot_global_sensitivity_analysis(cases_or_hosp: str, plot_df_: pd.DataFrame):
    ...
    # To fix the order of the bars
    if cases_or_hosp == 'cases':
        rank_dict = {'morbidity based with testing': 1, 'mortality based with testing': 2,
                     'morbidity based without testing': 3, 'mortality based without testing': 4}
    else:
        rank_dict = {'mortality based with testing': 1, 'morbidity based with testing': 2,
                     'mortality based without testing': 3, 'morbidity based without testing': 4}
    plot_df = plot_df_.copy()
    # specs argument - allows us to stretch on plot over more than one row/column in the figure
    fig = make_subplots(x_title='<b>Yearly averted {} / 1K individuals'
                        .format('reported cases' if cases_or_hosp=='cases' else 'hospitalizations'),
                        y_title= "<b>Probability that the tested strategy<br>will avert >= than X {}<br>per 1K individuals"
                        .format('reported cases' if cases_or_hosp=='cases' else 'hospitalizations')
                        )

    for i in range(0, len(plot_df.columns)):
        x = plot_df.index  # [::2]  # The dates of the model
        y = plot_df[plot_df.columns[i]].values  # The values of the current column
        if i == 0:  # Fixing the name of the plot
        ...
        fig.add_trace(go.Scatter(x=x, y=y, name=name,  hoverlabel=dict(namelength=-1), legendrank= rank_dict[name],
                                 line=dict(width=2.5, dash='dash' if (i==1 or i==3) else None,
                                           color='rgb(108,150,250)' if i < 2 else 'crimson'))
                          )
    # Adding Letter annotation for the paper
    fig.add_annotation(xref="paper", yref="paper", x= -0.12, y= 1.19,
                        text="<b> {}".format(LETTERS_FOR_FIGURES.pop(0)),
                        showarrow=False,
                        font=dict(
                            family="sans-serif",
    ...
    fig.update_layout(height=600, width=1100, template='plotly_white', margin=dict(l=200, b=90),
                      legend=dict(x=0.5, y=1, traceorder='normal', bgcolor='rgba(0,0,0,0)',
                                  font=dict(family='sans-serif', color='black', size=26)), font_family="sans-serif"
                      )

    fig.update_xaxes(ticks= 'outside', ticklen= 7, showline=True, linewidth=2, linecolor='black', showgrid= False,
                     nticks= 10, tickfont=dict(family='sans-serif', size= 28), tickformat= '.3f')#, range= [0, 20])
    # if cases_or_hosp == 'cases':
    #     fig.update_xaxes(range= [0, 100], dtick= 20)
    # else:
    #     fig.update_xaxes(range= [0, 1], dtick= 0.2)

    fig.update_yaxes(showline=True, linewidth=2, linecolor='black', showgrid= False, ticks='outside', ticklen=7,
                     range= [0, 1], tickfont=dict(family='sans-serif', size= 26), rangemode = "tozero",
                     scaleratio = 1)

    #fig.update_annotations(font_size=22)
    # Accessing directly to the subplots titles to change their size. essentially, they're annotations for plotly:
    # edit axis labels
    fig['layout']['annotations'][0]['font'] = {'size': 30}    # x_axis title size
    fig['layout']['annotations'][1]['font'] = {'size': 30}    # y_axis title size
    # Accessing directly to the y location of the x_title and change it:
    fig['layout']['annotations'][0]['yshift'] = -50
    # Accessing directly to the x location of the y_title and change it:
    fig['layout']['annotations'][1]['xshift'] = -70
    fig.write_html("Global_sensitivity_analysis_{}.html".format(cases_or_hosp))
    fig.write_image("Global_sensitivity_analysis_{}.jpeg".format(cases_or_hosp))
    fig.show()

def plot_panel_drop_from_baseline(
        scen: str              # To get the relevant path
):
    """This function gets a measurement & scen of model runs and plots a panel of bar plots
    such that rows are inventory levels (from low to high) and columns are health measurement (cases/hosp)
    ...
    # # To fix the order of the bars
    # rank_dict = {'mortality based':1, 'morbidity based':2, 'mortality based uniform':3,
    #              'morbidity based uniform':4}
    normalizing_factor = pop_israel / 1000      # to normalize cases and hosp to be per 1K individuals
    letters_for_new_Is = LETTERS_FOR_FIGURES[:3]
    letters_for_new_H = LETTERS_FOR_FIGURES[3:]

    # Reading the current drop from baseline file:
    with open('/Users/yotamdery/Old_Desktop/git/SEIR_model_COVID-main/Data/drop_from_baseline/'
              'drop_from_baseline_{}_{}.pickle'.format('new_Is', scen),'rb') as handle:
        drop_from_baseline_new_Is = pickle.load(handle)
    with open('/Users/yotamdery/Old_Desktop/git/SEIR_model_COVID-main/Data/drop_from_baseline/'
              'drop_from_baseline_{}_{}.pickle'.format('new_H', scen),'rb') as handle:
        drop_from_baseline_new_H = pickle.load(handle)

    # specs argument - allows us to stretch on plot over more than one row/column in the figure
    fig = make_subplots(rows=3, cols=2, #horizontal_spacing=0.05, vertical_spacing=0.07,
                        x_title="", specs=[[{"colspan": 1}, {"colspan": 1}],
                                          [{"colspan": 1}, {"colspan": 1}],
                                          [{"colspan": 1}, {"colspan": 1}]]
                        )

    ## Start plotting
    # Iterating over the health measurements:
    for k in range(len(['new_Is', 'new_H'])):
        # Fixing the current drop_from_baseline - for each iteration of k:
        drop_from_baseline = drop_from_baseline_new_Is if k == 0 else drop_from_baseline_new_H
        # Iterating over the inventory levels (Low, Medium, High):
        for i in range(3):
            middle_index = int(len(drop_from_baseline_new_Is[i]) / 2)
            # Converting the list of lists to list of dicts for the plot such that each dict is an inventory level:
            simulations_with_info = list_to_dict(drop_from_baseline[i][middle_index:],
                                                 dir_suffix_list[middle_index + 1:])
            ...
            for j in range(len(sorted_simulations_with_info)):
                ...
                # replacing the 'uniform' word to make the plot more indicative
                updated_curr_sim_name = curr_sim_name if 'uniform' not in curr_sim_name.split(' ') \
                    else curr_sim_name.replace("uniform", "<br>monthly vaccine administration")
                # Calculating the y value for the current bars
                curr_sim_val_with_info = list(sorted_simulations_with_info.values())[j]
                curr_sim_val_without_info = list(sorted_simulations_without_info.values())[j]

                # adding the traces - per bar each time
                fig.add_trace(go.Bar(name=updated_curr_sim_name,
                                     x=['<b>Direct information<br>(gained by testing<br>before vaccination)',
                                        '<b>Indirect information<br>(gained by analysis of<br>spatio-temporal dynamics)'],
                                     y=[curr_sim_val_with_info / normalizing_factor,
                                        curr_sim_val_without_info / normalizing_factor],
                                     # legendrank= rank_dict[curr_sim_name],
                                     hoverlabel=dict(namelength=-1), width=0.199, opacity=0.75,
                                     marker_color=traces_colors[curr_sim_name],
...
def plot_drop_from_baseline(
    ...
    """This function gets a measurement & years & scen of model runs and plots bar plot resembles the drop from the reduction in cases/hosp -
    compared to the baseline (benchmark sim), *stratified by the information*. We check the following simulations:
    1. morbidity_based,     2. mortality_based,     3. morbidity_based_uniform,     4. mortality_based_uniform"""
    dir_suffix_list = ['benchmark', 'morbidity_based', 'morbidity_based_uniform', 'mortality_based',
                       'mortality_based_uniform',
    ...
    letters_for_fig = LETTERS_FOR_FIGURES[:3] if measurement == 'new_Is' else LETTERS_FOR_FIGURES[3:]

    # Reading the current drop from baseline file:
    with open('/Users/yotamdery/Old_Desktop/git/SEIR_model_COVID-main/Data/drop_from_baseline/'
              'drop_from_baseline_{}_{}.pickle'.format(measurement, scen),'rb') as handle:
        drop_from_baseline = pickle.load(handle)
    ...
    for i in range(len(drop_from_baseline)):
        ...
        for j in range(len(sorted_simulations_with_info)):
            ...
            fig.add_trace(go.Bar(name= updated_curr_sim_name,
                                 x=['<b>Direct information<br>(gained by testing<br>before vaccination)',
                                    '<b>Indirect information<br>(gained by analysis of<br>spatio-temporal dynamics)'],
                                 y=[curr_sim_val_with_info/normalizing_factor,
                                    curr_sim_val_without_info/normalizing_factor],# legendrank= rank_dict[curr_sim_name],
                                 hoverlabel=dict(namelength=-1), width=0.199, marker_color=traces_colors[curr_sim_name],
                                 opacity=0.75)
                          )

        # Adding Letter annotation for the paper
        fig.add_annotation(xref="paper", yref="paper", x= -0.11, y= 1.25,
                           text="<b>{}".format(letters_for_fig[i]),
                           showarrow=False,
                           font=dict(
                               family="sans-serif",
                               size=39,
        ...
        # Fixing the Y-axis title dynamically:
        y_title = '<b>Yearly averted {}<br>/ 1K individuals'.format('cases' if measurement.split('_')[1] == 'Is' else
                                                              'hospitalizations')


        fig.update_layout(height=700 if measurement=='new_Is' else 700, width=950, barmode='group',
                          template='plotly_white', xaxis_title=dict(text='', font_size=30), #yaxis_tickformat='.0%',
                          yaxis_title= dict(text= y_title, font_size=26), title_font_color="black",
                          showlegend= False, yaxis_range=[0.0, np.max(curr_sim_val_with_info)/normalizing_factor],
                          )
                          ...
                          #     'yanchor': 'bottom'
                          # },
                          # legend=dict(title='', x=0.535, y=1.15, bgcolor='rgba(0,0,0,0)',
                          #             font=dict(family='sans-serif', color='black', size=24)
                          #             ),
                          # )
        fig.update_xaxes(showgrid= False,showticklabels=True,tickfont=dict(family='sans-serif',size= 24,color= "black"),
                        ticklen= 1, showline=True, linewidth=1., linecolor='black')
...
def plot_model_calibration_by_region(raw_data_per_region: pd.DataFrame, model_pred_per_region: pd.DataFrame):#, tot_pop):
    ...
    tot_cases_raw_data, tot_cases_model_pred = raw_data_per_region.T.sum(axis=1).to_frame(), \
                                               (model_pred_per_region.T.sum(axis=1) * pop_israel).to_frame()
    tot_cases_raw_data.columns, tot_cases_model_pred.columns = ['tot_cases_raw_data'], ['tot_cases_model']
    # Merging the data for the plots:
    raw_data_model_merged = tot_cases_raw_data.join(tot_cases_model_pred).set_index([[str(i) for i in range(1, 31)]])
    raw_data_model_merged.index = raw_data_model_merged.index.astype(str)
    raw_data_model_merged_sorted = raw_data_model_merged.sort_values(by= 'tot_cases_raw_data', ascending= False)
    fig = go.Figure()
    # Adding the raw data bars
    fig.add_trace(go.Bar(x= raw_data_model_merged_sorted.index,
                         y= raw_data_model_merged_sorted['tot_cases_raw_data'].values, hoverlabel=dict(namelength=-1),
                         width=0.35, name='Raw data', marker_color='black', opacity=0.75))
    # Adding the model predictions bars
    fig.add_trace(go.Bar(x= raw_data_model_merged_sorted.index,
                         y= raw_data_model_merged_sorted['tot_cases_model'].values, hoverlabel=dict(namelength=-1),
                         width=0.35, name='Model predictions', marker_color='rgba(255, 82, 93, .9)', opacity=0.75))
    # Adding Letter annotation for the paper
    fig.add_annotation(xref="paper", yref="paper", x=-0.07, y=1.15,
                       text="<b> C",
                       showarrow=False,
                       font=dict(
                           family="sans-serif",
    ...
    fig.update_layout(height=600, width=1250, barmode='group', template='plotly_white',
                      xaxis_title=dict(text='<b>Region id', font_size=28),
                      yaxis_title=dict(text="<b> Daily reported cases [#]", font_size=24),
                      # title={
                      ...
                      #     'yanchor': 'bottom'
                      # },
                      legend=dict(title='', x= 0.735, y= 1., traceorder='normal',
                                  bgcolor='rgba(0,0,0,0)', #bordercolor='black', borderwidth=0.5,
                                  font=dict(family='sans-serif', color='black', size=28)),
                      bargap=.2, bargroupgap=.1#, legend_title_font_size=20
                      )
    fig.update_xaxes(showticklabels=True, tickangle= 0, tickfont=dict(family='sans-serif', size=22), ticks= 'outside',
                     ticklen= 4, tickwidth= 2, showline=True, linewidth=1.5, linecolor='black', showgrid= False)
    fig.update_yaxes(showline = True, linewidth = 1.5, linecolor = 'black', showticklabels=True, ticks= 'outside',
                     ticklen= 4, tickwidth= 2, tickfont=dict(family='sans-serif', size=24), showgrid= False)
...
def plot_model_calibration_by_age(raw_data_per_age: pd.DataFrame, model_pred_per_age: pd.DataFrame,
    ...
    # Scaling the model's prediction to be on the same units:
    #model_pred_per_age_copy *= pop_israel
    fig = go.Figure()

    # Adding the raw data bars
    fig.add_trace(go.Bar(x=raw_data_per_age_copy.index, y=raw_data_per_age_copy['tot_cases'],
                         hoverlabel=dict(namelength=-1), width=0.21, name='Raw data', marker_color='black',
                         opacity=0.75))
    # Adding the model predictions bars
    fig.add_trace(go.Bar(x=model_pred_per_age_copy.index, y=model_pred_per_age_copy['tot_cases'],
                         hoverlabel=dict(namelength=-1), width=0.21, name='Model predictions',
                         marker_color='rgba(255, 82, 93, .9)', opacity=0.75))
    # Adding Letter annotation for the paper
    fig.add_annotation(xref="paper", yref="paper", x=-0.29, y=1.15,
                       text="<b> B",
                       showarrow=False,
                       font=dict(
                           family="sans-serif",
    ...
    if with_norm:
        ...
        pass
    fig.update_layout(height=600, width=450, template='plotly_white', showlegend= False,
                      xaxis_title=dict(text='<b>Age [y]', font_size=24),
                      yaxis_title=dict(text="<b> Daily reported cases [#]", font_size=24),
                      # title={
    ...
    fig.update_layout(barmode='group', bargap=.6, bargroupgap=.1)
    fig.update_xaxes(showticklabels=True, tickfont=dict(family='sans-serif', size= 24), ticks= 'outside', ticklen= 4,
                     tickwidth= 2, showline=True, linewidth= 1.5, linecolor='black', showgrid= False)
    fig.update_yaxes(showline = True, linewidth = 1.5, linecolor = 'black', ticks= 'outside', ticklen= 4, tickwidth= 2,
                     tickfont=dict(family='sans-serif', size=24), showgrid= False)
        ...
        for j in range(len(start_date_vline)):  # A loop to add Vlines for each relevant event
            ...
            fig.add_vline(x=start_date_vline[j], line_color=vline_color, line_dash='dash', row=i + 1, col=1) \
                ...
                .add_vrect(x0=start_date_vline[j], x1=end_date_vline[j],
                           ...
                           line_width=0, annotation=dict(font_size=12, font_family="sans-serif"),
                           row=i + 1, col=1)
...
def plot_calibrated_nationwide_model(
    ...
    fig.add_trace(go.Scatter(x=x_axis[1::2], y=raw_data_series_odd_pos, hoverlabel=dict(namelength=-1),
                             mode='markers', name='Raw data', marker=dict(size=3.5, color='darkblue', line_width=1)))
    fig.add_trace(go.Scatter(x=x_axis, y=model_data_pred_series, hoverlabel=dict(namelength=-1),
                             line=dict(width=4., color='rgba(255, 82, 93, .9)'), name='Model predictions'))
    # Adding Letter annotation for the paper
    fig.add_annotation(xref= "paper", yref= "paper", x=-0.065, y=1.1,
                       text="<b> A",
                       showarrow=False,
                       font=dict(
                           family="sans-serif",
    ...
    # Updates
    fig.update_layout(height=600, width=1600, xaxis_title="<b> Date", yaxis_title="<b> Daily reported cases [#]",
                      font=dict(size=20), font_family="sans-serif", template='plotly_white',
                      # title={
                      ...
                      #           'yanchor': 'bottom'
                      #       },
                      legend=dict(title='', traceorder='normal', bordercolor='black', bgcolor='rgba(0,0,0,0)',
                                  x=0.01, y=1., #borderwidth=0.8,
                                  font=dict(family='sans-serif', color='black', size= 26))#, legend_title_font_size=16
                      )
    fig.update_xaxes(range=[x_axis.min(), x_axis.max()], showline= True, linewidth= 1.5, linecolor= 'black',
                     ticks= 'outside', showticklabels=True, ticklen= 4, tickwidth= 2, tickfont_size= 24,
                     tickfont_family="sans-serif", nticks= 6, showgrid= False)
    fig.update_yaxes(showline=True, linewidth=1.5, linecolor='black',  showticklabels=True, ticks= 'outside',
                     ticklen= 4, tickwidth= 2, tickfont_size= 24, tickfont_family="sans-serif", showgrid= False,
                     rangemode= 'nonnegative')
    fig.show()
    fig.write_image("country_level_calibration.jpg")


def plot_respiration_cases(res_mdl, days=None):
    fig, ax = plt.subplots(figsize=(15, 10))
    ax.plot(((res_mdl['Vents']).sum(axis=1)) * pop_israel)
    ax.set_ylabel('Respiratory cases [#]', fontsize=35)
